...
export default function Dashboard() {
    ...
    const [filterStage, setFilterStage] = useState('all'); // Stage filter: all, 1, 2, 3, 4, 5
    ...
    const filteredContracts = contracts.filter(c => {
        ...
        // Stage Filter
        let matchesStage = filterStage === 'all' || c.stage.toString() === filterStage;
    ...
    return (
        <div className="space-y-8">
            {/* Header & Stats */}
            <div>
                <h2 className="text-4xl font-bold text-slate-900 mb-6">Dashboard Overview</h2>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center space-x-4">
                        <div className="p-3 bg-indigo-50 text-indigo-600 rounded-lg"><FileText size={24} /></div>
                        <div>
                            <p className="text-slate-500 text-base font-medium">Total Contracts</p>
                    ...
                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center space-x-4">
                        ...
                        <div>
                            <p className="text-slate-500 text-base font-medium">Pending Action</p>
                    ...
                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center space-x-4">
                        ...
                        <div>
                            <p className="text-slate-500 text-base font-medium">Completed</p>
                    ...
                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center space-x-4">
                        ...
                        <div>
                            <p className="text-slate-500 text-base font-medium">Nav. Attention</p>
                            <p className="text-3xl font-bold text-slate-900">{stats.attention}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                ...
                <div className="flex items-center space-x-3">
                    ...
                    <select
                        ...
                        className="bg-slate-50 border border-slate-200 text-slate-900 text-base rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block p-2.5 outline-none font-medium cursor-pointer hover:bg-slate-100 transition-colors"
                    >
                        <option value="pending">Pending</option>
                        <option value="approved">Approved</option>
                        <option value="rejected">Denied</option>
                    </select>
                    <span className="text-base font-medium text-slate-500 ml-4">Stage:</span>
                    <select
                        value={filterStage}
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {filteredContracts.map((c, idx) => (
                            <tr key={`${c.contract_id}-${c.lot_id || 'main'}-${idx}`} className="hover:bg-indigo-50/30 transition-colors group cursor-default">
                                ...
                                <td className="px-6 py-4 font-medium text-slate-900 text-base">{c.vendor_name}</td>
                                <td className="px-6 py-4 text-slate-500 text-base">{c.gst_number || '-'}</td>
                                        ...
                                        {[
                                            ...
                                            return (
                                                <React.Fragment key={step.id}>
                                                    <div className={`px-2 py-1 rounded text-xs uppercase tracking-wide border ${colorClass}`}>
                                                    {step.label}
                                                    </div>
                                ...
                                <td className="px-6 py-4 text-right">
                                    <button
                                        ...
                                        className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-3 rounded-lg font-medium text-base transition-all flex items-center gap-2 ml-auto"
                                    >
                                        {c.stage === 1 ? 'Review Contract' :
                                         c.stage === 2 ? 'Quality Entry' :
                                         c.stage === 3 ? 'Sampling' :
                                         c.stage === 4 ? 'CTL Entry' :
                                         c.stage === 5 ? 'Payment' : 'View'}
                                        <ArrowRight size={16} />
                                    </button>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td colSpan="6" className="px-6 py-16 text-center text-slate-400">
                                    <div className="flex flex-col items-center justify-center">
                                        ...
                                        <p className="text-xl font-medium text-slate-500">No contracts found</p>
